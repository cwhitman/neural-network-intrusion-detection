...
"""Contains functions designed to parse pcap files using scapy-python3: https://github.com/phaethon/scapy.
   ...
   After initialization, parsed, normalized pcap file can be accessed through the field pcap_dictionary.
   """
class pcapParser():
    ...
    def __init__(self,pcap_file,*pcap_files):
        self.pcap_dictionary = {}
        self.pcap_dictionary_data = {}
        self.addFileToDictionary(pcap_file,*pcap_files)



    """Updates the connections last hour and connections this month values in the pcap_dictionary.
       ...
       Args:
        ...
        Retuns: self.pcap_dictionary"""
    def updateConnectionTimes(self,time):
        for key in self.pcap_dictionary:
            if __name__ == '__main__':
                if(time - self.pcap_dictionary_data[key][0] > 3600): #total number of connections last hour
                    self.pcap_dictionary_data[key][0]=time
                    self.pcap_dictionary[key][3] = 0
                if (time - self.pcap_dictionary_data[key][1] > 3600):  # total number of connections last month
                    self.pcap_dictionary_data[key][1] = time
                    self.pcap_dictionary[key][4] = 0

    """Adds the pcap files to self.pcap_dictionary.
       self.pcap_dictionary is a dictionary where key is the source IP and value is
       ...
       Total number of connections last hour, Total number of connections this month].
        self.pcap_dictionary_data is used to keep track of things such as hour_last_reset for each entry.
        It is organized as { ip, [hour_last_reset,month_last_reset] }

        Assuming that dictionary lookup time is constant, this operation is O(n+m+p) where n is the previous size of the dictionary,
        m is the number of packets in the pcap_files, and p is the size of the resulting dictionary.
       Args:
            pcap_files: Any number of valid pcap file names.
        Retuns: self.pcap_dictionary"""
    def addFileToDictionary(self,*pcap_files):
        self.__unNormalizeData__() # un-nomormalize the data so that it can be edited.
        for pcap_file in pcap_files:
            with PcapReader(pcap_file) as pcap_reader:
                ...
                for packet in pcap_reader:
                    ...
                    if(src not in self.pcap_dictionary): #Add ip to dictionary
                        self.pcap_dictionary[src] = [packet.time,0.0,0.0,0.0,0.0]
                        self.pcap_dictionary_data[src] = [float(packet.time),float(packet.time)]

                    list = self.pcap_dictionary[src]
                    list_data = self.pcap_dictionary_data[src]
        ...
        self.__normalizeData__() #normalize the data
        return self.pcap_dictionary



    """Normalizes all values inside the dictionary using the function 1/(1/+x).
        Args:
            dictionary: dictionary produced using addFileToDictionary. Defaults to self.pcap_dictionary.
        Returns:
            The same dictionary with all values normalized."""
    def __normalizeData__(self,dictionary = None):
        if(dictionary is None):
            dictionary = self.pcap_dictionary
        for key in dictionary:
             dictionary[key]= [1/(1+x) for x in dictionary[key]]
        return dictionary
    ...
    def __unNormalizeData__(self, dictionary=None):
        if (dictionary is None):
            dictionary = self.pcap_dictionary
        for key in dictionary:
            dictionary[key] = [1 /x -1 for x in dictionary[key]]
        return dictionary
